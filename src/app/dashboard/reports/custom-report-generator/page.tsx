...
export default function CustomReportGeneratorPage() {
    ...
    const savedReports = [
        ...
        { id: 2, name: "Rapport du marché de l'emploi Q4 2023", type: "Analyses d'emploi", dateGenerated: "2024-01-01", status: "ready", format: "Excel" },
        { id: 3, name: "Rapport d'engagement d'entreprise", type: "Analyses d'entreprise", dateGenerated: "2024-01-12", status: "generating", format: "PDF" }
    ];
            ...
            <div className="grid gap-8 lg:grid-cols-2">
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-2"><label className="text-sm font-medium">Type de rapport</label><Select value={reportType} onValueChange={setReportType}><SelectTrigger><SelectValue placeholder="Sélectionner le type de rapport" /></SelectTrigger><SelectContent><SelectItem value="user_analytics">Analyses d'utilisateurs</SelectItem><SelectItem value="job_analytics">Analyses d'emploi</SelectItem><SelectItem value="company_analytics">Analyses d'entreprise</SelectItem><SelectItem value="engagement_analytics">Analyses d'engagement</SelectItem><SelectItem value="financial_analytics">Analyses financières</SelectItem></SelectContent></Select></div>
                        ...
                        <div className="space-y-3"><label className="text-sm font-medium">Sélectionner les métriques</label><div className="space-y-3 max-h-64 overflow-y-auto">{availableMetrics.map((metric) => (<div key={metric.id} className="flex items-center space-x-2"><Checkbox id={metric.id} checked={selectedMetrics.includes(metric.id)} onCheckedChange={() => handleMetricToggle(metric.id)} /><label htmlFor={metric.id} className="text-sm font-normal">{metric.label}<span className="text-muted-foreground ml-2">({metric.category})</span></label></div>))}</div></div>
                        <div className="flex gap-3">
                            ...
                            <Button variant="outline"><Eye className="h-4 w-4 mr-1" />Aperçu</Button>
                        </div>
                    </CardContent>
                </Card>
                <Card>
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {savedReports.map((report) => (<div key={report.id} className="border rounded-lg p-4"><div className="flex items-start justify-between mb-3"><div><h4 className="font-medium">{report.name}</h4><p className="text-sm text-muted-foreground">{report.type}</p></div><Badge variant={report.status === 'ready' ? 'default' : 'secondary'} className={report.status === 'ready' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : ''}>{report.status === 'ready' ? 'Prêt' : 'En génération'}</Badge></div><div className="flex items-center justify-between text-sm text-muted-foreground mb-3"><div className="flex items-center gap-2"><Calendar className="h-4 w-4" /><span>Généré: {report.dateGenerated}</span></div><span>Format: {report.format}</span></div>{report.status === 'ready' && (<div className="flex gap-2"><Button variant="outline" size="sm" className="flex-1"><Eye className="h-4 w-4 mr-1" />Voir</Button><Button variant="outline" size="sm" className="flex-1"><Download className="h-4 w-4 mr-1" />Télécharger</Button></div>)}</div>))}
                        </div>
                    </CardContent>
                </Card>
            </div>
